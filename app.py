...
@st.cache_resource
def load_llm():
    llm = LLM(model="llama-3.3-70b-versatile", api_key=groq_api_key)
    # llm = LLM(
...
with st.sidebar:
    ...
    st.session_state.bright_data_api_key = st.text_input("BRIGHT DATA API Key", type="password")
    ...
    st.divider()
    condition_met = st.session_state.bright_data_api_key and st.session_state.groq_api_key and st.session_state.youtube_channels[0]
    # Disable button if condition is False
    if condition_met:
        if st.button("Start Analysis 🚀", type="primary", on_click=start_analysis):
            pass
    else:
        st.warning("Enter the Channels and API Keys.")
